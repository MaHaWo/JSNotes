...
* Python source files:
  * The Python package source files are located in the `jsnotes` directory.
  * The file `jsnotes.__main__.py` defines a command line interface that
    is accessible both as the command `jsnotes` and via
    `python -m jsnotes`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_jsnotes.py` contains the unit tests for the package.
